...
### Your First Request
```bash
curl "http://your-api-domain.com/api/v1/air-quality/?lat=34.05&lon=-118.24"
```
...
**Basic Request:**
```bash
curl "http://your-api-domain.com/api/v1/air-quality/?lat=40.71&lon=-74.01"
```

**With Forecast:**
```bash
curl "http://your-api-domain.com/api/v1/air-quality/?lat=40.71&lon=-74.01&include_forecast=true"
```

**Custom Search Radius:**
```bash
curl "http://your-api-domain.com/api/v1/air-quality/?lat=37.77&lon=-122.42&radius_km=10"
```

**Fresh Data (Skip Cache):**
```bash
curl "http://your-api-domain.com/api/v1/air-quality/?lat=34.05&lon=-118.24&no_cache=true"
```
...
**Good Air Quality:**
```bash
curl "http://your-api-domain.com/api/v1/health-advice/?aqi=45"
```
...
```bash
curl "http://your-api-domain.com/api/v1/health-advice/?aqi=165"
```
...
```bash
curl "http://your-api-domain.com/api/v1/sources/"
```
...
```bash
curl "http://your-api-domain.com/api/v1/health/"
```
...
async function getCurrentAirQuality(lat, lon) {
  const response = await fetch(
    `http://your-api-domain.com/api/v1/air-quality/?lat=${lat}&lon=${lon}`
  );
...
async function getAirQualityForecast(lat, lon) {
  const response = await fetch(
    `http://your-api-domain.com/api/v1/air-quality/?lat=${lat}&lon=${lon}&include_forecast=true`
  );
  const data = await response.json();
...
async function addAirQualityMarker(map, lat, lon) {
  ...
  const adviceResponse = await fetch(
    `http://your-api-domain.com/api/v1/health-advice/?aqi=${data.current.aqi}`
  );
...
```javascript
// Vanilla JavaScript
fetch('http://your-api-domain.com/api/v1/air-quality/?lat=34.05&lon=-118.24')
...
function AirQualityWidget({ lat, lon }) {
  ...
  useEffect(() => {
    fetch(`http://your-api-domain.com/api/v1/air-quality/?lat=${lat}&lon=${lon}`)
      .then(res => res.json())
...
def get_air_quality(lat, lon):
    url = f"http://your-api-domain.com/api/v1/air-quality/"
...
curl -H "Accept: application/json" \
     "http://your-api-domain.com/api/v1/air-quality/?lat=40.71&lon=-74.01"

# Pretty print
curl "http://your-api-domain.com/api/v1/air-quality/?lat=37.77&lon=-122.42" | jq
```
